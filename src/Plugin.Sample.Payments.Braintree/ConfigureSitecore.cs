...
namespace Plugin.Sample.Payments.Braintree
{
    ...
    public class ConfigureSitecore : IConfigureSitecore
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddTransient(typeof(IBraintreeGateway), typeof(BraintreeGateway));

            services.Sitecore().Pipelines(config => config
                .ConfigurePipeline<IGetClientTokenPipeline>(d => { d.Add<GetClientTokenBlock>(); })
                .ConfigurePipeline<ICreateOrderPipeline>(d =>
                {
                    d.Add(provider => new CreateFederatedPaymentBlock(provider.GetRequiredService<IRetryerRegistry>(), provider.GetRequiredService<IBraintreeGateway>()))
