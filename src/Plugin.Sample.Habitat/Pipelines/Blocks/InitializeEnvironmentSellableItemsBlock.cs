...
namespace Plugin.Sample.Habitat
{
    ...
    public class InitializeEnvironmentSellableItemsBlock : AsyncPolicyTriggerConditionalPipelineBlock<string, string>
    {
        ...
        private async Task UpsertSellableItem(SellableItem item, CommercePipelineExecutionContext context)
        {
            ...
            var entity = await _findEntityPipeline.RunAsync(new FindEntityArgument(typeof(SellableItem), item.Id), context).ConfigureAwait(false);
            if (entity == null)
            {
                await _persistEntityPipeline.RunAsync(new PersistEntityArgument(item), context).ConfigureAwait(false);
                return;
            }
                ...
                foreach (var variation in item.GetComponent<ItemVariationsComponent>().ChildComponents.OfType<ItemVariationComponent>())
                {
                    ...
                    if (existingVariation != null)
                    {
                        ...
                        foreach (var policy in variation.Policies)
                        {
                            ...
                            existingVariation.AddOrUpdatePolicy(policy);
                        }
                    }
                    else
                    {
                        variations.AddChildComponent(variation);
                    }
                }
            }

            await _persistEntityPipeline.RunAsync(new PersistEntityArgument(existingSellableItem), context).ConfigureAwait(false);
        }
    }
}
